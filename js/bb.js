d3.csv("data/bb.csv", cast, function(data) {
  ...
  var ubsvg = d3.select("#bbgraph").append("svg")
   ...
   bbrects = gymnast.selectAll("rect.bb")
    ...
    .enter().append("rect")
      ...
      .attr("class", function(d) {        
        ...
        return "bars bb class" + classLabel;
      })
  ...
  .property("disabled", true);
          d3.select(this)           
            ...
            d3.select("#bblabel").select("#sortbb").property("disabled", false)
            d3.select("#bblabel").style("color", "black")
            //sort the bars if checkbox is clicked            
            d3.select("#sortbb").on("change", function() {
              changevt(dataset)});  
          ...
          if (active_link === this.id.split("id").pop()) {//active square selected; turn it OFF
            ...
            if (d3.select("#bblabel").select("#sortbb").property("checked")) {              
              restoreXFlag = true;
            }
  ...
  .property("disabled", false);
      //restore graph after a single selection
      d3.selectAll(".bars.bb:not(.class" + class_keep + ")")
            .transition()
            .duration(1000)
            .delay(function() {
      ...
      d3.selectAll(".bb.class" + class_keep)
    ...
    if ($('#sortbb').is(":checked")) sortDescending = true;
    ...
    gymnast.selectAll("bb.class" + active_link)
         .sort(function(a, b) { 
    ...
    //sort bars
    transition.selectAll(".bb.class" + active_link)
      .delay(delay)
      .attr("x", function(d) {      
    ...
    var fixxaxis = transition.select(".bb.x.axis")
        .call(vtxAxis)
    	.selectAll(".tick text")
